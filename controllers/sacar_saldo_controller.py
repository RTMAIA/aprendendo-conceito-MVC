...
class SacarSaldoController:

    def sacar(self, numero_da_conta, valor, salvar=True):
        try:
            ...
            x = list(map(lambda x: Conta(self.conta[0].nome, self.conta[0].cpf, self.conta[0].numero_da_conta, self.conta[0].data_nascimento, self.conta[0].extrato, saldo) if (
                        x.numero_da_conta == str(numero_da_conta)) else (x), x))
            ...
            with open('contas.txt', 'w') as arq:
                for i in x:
                    ...
                    arq.writelines('\n')
            if salvar == True:
                a = Extrato('Saque', valor, self.conta[0].nome, self.conta[0].cpf, self.conta[0].numero_da_conta)
